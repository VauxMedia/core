...
func Test_Static(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://localhost:3000/core.go", nil)
	if err != nil {
		t.Error(err)
	}
...
func Test_Static_Head(t *testing.T) {
	...
	req, err := http.NewRequest("HEAD", "http://localhost:3000/core.go", nil)
	if err != nil {
		t.Error(err)
	}
...
func Test_Static_As_Post(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "http://localhost:3000/core.go", nil)
	if err != nil {
		t.Error(err)
	}
...
func Test_Static_Options_Logging(t *testing.T) {
	...
	m := &Core{Injector: inject.New(), action: func() {}, logger: log.New(&buffer, "[core] ", 0)}
	m.Map(m.logger)
	...
	m.ServeHTTP(response, req)
	expect(t, response.Code, http.StatusOK)
	expect(t, buffer.String(), "[core] [Static] Serving /core.go\n")
...
func Test_Static_Options_ServeIndex(t *testing.T) {
	...
	opt := StaticOptions{IndexFile: "core.go"} // Define core.go as index file
	m.Use(Static(currentRoot, opt))
...
func Test_Static_Options_Prefix(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "http://localhost:3000/public/core.go", nil)
	if err != nil {
		t.Error(err)
	}
...
func Test_Static_Options_Fallback(t *testing.T) {
	...
	// Serve current directory under /public
	m.Use(Static(currentRoot, StaticOptions{Fallback: "/core.go"}))
